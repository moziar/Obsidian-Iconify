...
class BlankLine extends Node$1 {
  constructor() {
...
class CollectionItem extends Node$1 {
  ...
  parse(context, start) {
    ...
    let offset = Node$1.endOfWhiteSpace(src, start + 1);
    let ch = src[offset];
    ...
    while (ch === '\n' || ch === '#') {
      if (ch === '#') {
        const end = Node$1.endOfLine(src, offset + 1);
        comments.push(new Range(offset, end));
        offset = end;
      } else {
        atLineStart = true;
        lineStart = offset + 1;
        const wsEnd = Node$1.endOfWhiteSpace(src, lineStart);
        if (src[wsEnd] === '\n' && comments.length === 0) {
...
let Collection$1 = class Collection extends Node$1 {
  static nextContentHasIndent(src, offset, indent) {
    const lineStart = Node$1.endOfLine(src, offset) + 1;
  ...
  parse(context, start) {
    ...
    let lineStart = Node$1.startOfLine(src, start);
    ...
    let ch = src[offset];
    let atLineStart = Node$1.endOfWhiteSpace(src, lineStart) === offset;
    ...
    while (ch) {
      while (ch === '\n' || ch === '#') {
        ...
        if (Node$1.atBlank(src, offset)) {
          const wsEnd = Node$1.endOfWhiteSpace(src, offset);
...
class Directive extends Node$1 {
  constructor() {
...
let Document$2 = class Document extends Node$1 {
  static startCommentOrEndBlankLine(src, start) {
    const offset = Node$1.endOfWhiteSpace(src, start);
    const ch = src[offset];
    return ch === '#' || ch === '\n' ? offset : start;
  }
  constructor() {
  ...
  parseContents(start) {
    ...
    let offset = Node$1.endOfWhiteSpace(src, start);
    ...
    while (!Node$1.atDocumentBoundary(src, offset, Char.DOCUMENT_END)) {
      switch (src[offset]) {
        ...
        default:
          {
            const iEnd = Node$1.endOfIndent(src, offset);
            const context = {
              atLineStart,
              indent: -1,
              inFlow: false,
...
let Alias$1 = class Alias extends Node$1 {
  /**
  ...
  parse(context, start) {
    ...
    let offset = Node$1.endOfIdentifier(src, start + 1);
...
class BlockValue extends Node$1 {
  constructor(type, props) {
    super(type, props);
  ...
  get strValue() {
    ...
    for (let i = start; i < end; ++i) {
      ...
      if (ch === '\n') {
        ...
        const lineEnd = Node$1.endOfLine(src, i);
  ...
  parseBlockValue(start) {
    ...
    for (let ch = src[offset]; ch === '\n'; ch = src[offset]) {
      ...
      const end = Node$1.endOfBlockIndent(src, indent, offset); // should not include tab?
      if (end === null) break;
      const ch = src[end];
...
class FlowCollection extends Node$1 {
  constructor(type, props) {
    super(type, props);
    this.items = null;
  }
  ...
  toString() {
    ...
    if (value != null) return value;
    const nodes = items.filter(item => item instanceof Node$1);
    let str = '';
...
class QuoteDouble extends Node$1 {
    ...
    for (let i = start + 1; i < end - 1; ++i) {
      ...
      if (ch === '\n') {
        ...
        const {
          ...
          offset,
          error
        } = Node$1.foldNewline(src, i, indent);
        str += fold;
        i = offset;
...
class QuoteSingle extends Node$1 {
  static endOfQuote(src, offset) {
    let ch = src[offset];
    while (ch) {
      if (ch === "'") {
...
class ParseContext {
  ...
  constructor(orig = {}, {
    ...
    _defineProperty(this, "parseNode", (overlay, start) => {
      if (Node$1.atDocumentBoundary(this.src, start)) return null;
  ...
  parseProps(offset) {
    ...
    while (ch === Char.ANCHOR || ch === Char.COMMENT || ch === Char.TAG || ch === '\n') {
      if (ch === '\n') {
        ...
        let end = Node$1.endOfIdentifier(src, offset + 1);
        if (ch === Char.TAG && src[end] === ',' && /^[a-zA-Z0-9-]+\.[a-zA-Z0-9-]+,\d\d\d\d(-\d\d){0,2}\/\S/.test(src.slice(offset + 1, end + 13))) {
          ...
          end = Node$1.endOfIdentifier(src, end + 5);
        }
        props.push(new Range(offset, end));
        lineHasProps = true;
        offset = Node$1.endOfWhiteSpace(src, end);
      }
      ch = src[offset];
    }
...
var elementToPath_umdExports = elementToPath_umd.exports;
var toPath = /*@__PURE__*/getDefaultExportFromCjs(elementToPath_umdExports);

var svgPathTools_umd = {exports: {}};
...
} (svgPathTools_umd, svgPathTools_umd.exports));

var svgPathTools_umdExports = svgPathTools_umd.exports;
...
function scalePath(node, scaleOptions) {
    ...
    if (/(rect|circle|ellipse|polygon|polyline|line|path)/.test(o.name)) {
        const path = toPath(o);
        const parseD = svgPathTools_umdExports.parse(path);
        const scaleD = svgPathTools_umdExports.scale(parseD, scaleOptions);
        const d = svgPathTools_umdExports.stringify(scaleD);
...
function svgToIcon(value) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const parsed = yield svgson_umdExports.parse(value);
            const maxViewBox = getMaxViewBox(parsed);
            const children = [];
            if (maxViewBox) {
                parsed.children.forEach((path) => {
                    children.push(svgson_umdExports.stringify(
...
class IconManager {
    ...
    loadIcons() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const icons = data.icons || data; // 兼容旧版本数据格式
            ...
            for (const icon in customIcons) {
                obsidian.addIcon(icon, customIcons[icon]);
            }
        });
    }
    setIcon(opts) {
    ...
    addCustomIcon(name, svg) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!name || !svg)
                return false;
            try {
                const iconSVG = (yield svgToIcon(svg)) || "";
                obsidian.addIcon(name, iconSVG);
...
class DefaultIconsPage extends obsidian.SettingPage {
    constructor(plugin) {
        super();
        this.plugin = plugin;
    }
...
class IconSwapperPlugin extends obsidian.Plugin {
    onload() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const saveIcons = (data) => __awaiter(this, void 0, void 0, function* () { return yield this.saveData(data); });
...
class ExportModal extends obsidian.Modal {
    ...
    onOpen() {
        ...
        new obsidian.Setting(contentEl)
            ...
            setting.controlEl.createEl("a", { cls: "icon-swapper-copy", text: "Copy to clipboard", href: "#" }, (copyButton) => {
            ...
            setting.controlEl.createEl("input", {
                ...
                importInput.addEventListener("change", (e) => {
                    ...
                    reader.onload = (e) => __awaiter(this, void 0, void 0, function* () {
                        ...
                        if (result) {
                            yield importAndClose(result.toString().trim());
                        }
                    });
...
// ========== Custom Icon Modals ==========
class AddCustomIconModal extends obsidian.Modal {
    ...
    onOpen() {
        ...
        // Icon Name
        new obsidian.Setting(contentEl).setName("Icon name").addText((text) => {
            this.iconNameInput = text;
            text.setPlaceholder("e.g. my-icon").setValue("");
        });
        // SVG source — Upload
        const uploadSetting = new obsidian.Setting(contentEl).setName("SVG source");
        ...
        fileInput.addEventListener("change", (event) => {
            ...
            if (file && file.type === "image/svg+xml") {
                ...
                reader.onload = (e) => {
                    ...
                    this.updatePreview();
                    new obsidian.Notice(`SVG file "${file.name}" loaded.`);
                };
        ...
        // SVG source — Paste
        new obsidian.Setting(contentEl).setName("Or paste SVG").then((setting) => {
            new obsidian.TextAreaComponent(contentEl)
...
class UpdateCustomIconModal extends obsidian.Modal {
    constructor(app, plugin, iconName, onSave) {
        super(app);
        this.currentSvg = "";
        this.plugin = plugin;
    ...
    onOpen() {
        ...
        new obsidian.Setting(contentEl).setName("Current icon").then((setting) => {
            setting.controlEl.createDiv({ cls: "icon-swapper-icon" }, (icon) => {
                try {
                    obsidian.setIcon(icon, this.iconName);
                }
                catch (e) {
                    icon.setText("?");
                }
        ...
        // SVG source — Upload
        const uploadSetting = new obsidian.Setting(contentEl).setName("New SVG source");
        const fileInput = uploadSetting.controlEl.createEl("input", {
            attr: { type: "file", accept: ".svg", style: "display: none;" },
        });
        uploadSetting.addButton((button) => {
    ...
    getSettingDefinitions() {
        return [
            // Import/Export/Revert
            {
                name: "Import/Export",
                render: (setting) => {
                    setting.controlEl.createEl("a", { cls: "icon-swapper-import", text: "Import", href: "#" }, (el) => {
                        el.addEventListener("click", (e) => {
                            e.preventDefault();
                            new ImportModal(this.app, this.plugin).open();
                        });
                    });
                    setting.controlEl.createEl("a", { cls: "icon-swapper-export", text: "Export", href: "#" }, (el) => {
                        el.addEventListener("click", (e) => {
                            e.preventDefault();
            ...
            // Custom Icon — list
            {
                ...
                addItem: {
                    name: "Add icon",
                    action: () => {
                        const modal = new AddCustomIconModal(this.app, this.plugin, (name, svg) => __awaiter(this, void 0, void 0, function* () {
                            const success = yield this.plugin.iconManager.addCustomIcon(name, svg);
                            if (success) {
                                new obsidian.Notice(`Icon ${name} added.`);
                                this.update();
                            }
                            else {
                                new obsidian.Notice("Failed to add icon.");
                            }
                        }));
                        modal.open();
                    },
                },
                onDelete: (idx) => __awaiter(this, void 0, void 0, function* () {
                    const names = Object.keys(this.plugin.iconManager.customIcons);
                    const name = names[idx];
                    if (name) {
                        yield this.plugin.iconManager.removeCustomIcon(name);
                        new obsidian.Notice(`Icon ${name} deleted.`);
                        this.update();
                    }
                }),
                items: Object.keys(this.plugin.iconManager.customIcons).map((iconName) => ({
                    name: iconName,
                    searchable: false,
                    render: (setting) => {
                        ...
                        setting.nameEl.createDiv({ cls: "icon-swapper-container" }, (container) => {
                            container.createDiv({ cls: "icon-swapper-icon" }, (icon) => {
                                try {
                                    obsidian.setIcon(icon, capturedName);
                                }
                                catch (e) {
                                    icon.setText("?");
                                }
                            });
                            container.createDiv({ cls: "icon-swapper-name" }, (icoName) => {
                        ...
                        setting.addButton((button) => {
                            button
                                ...
                                .onClick(() => {
                                const modal = new UpdateCustomIconModal(this.app, this.plugin, capturedName, (svg) => __awaiter(this, void 0, void 0, function* () {
                                    const success = yield this.plugin.iconManager.addCustomIcon(capturedName, svg);
                                    if (success) {
                                        new obsidian.Notice(`Icon ${capturedName} updated.`);
                                        this.update();
                                    }
                                    else {
